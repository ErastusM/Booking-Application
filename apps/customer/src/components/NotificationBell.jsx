...
const NotificationBell = ({ isTransparent }) => {
    ...
    const [busyId, setBusyId] = useState(null); // notification currently being deleted — guards double-clicks
    ...
    return (
        <div ref={dropdownRef} style={{ position: 'relative' }}>
            {/* Bell button — explicit svg size + inline badge styles: this app has
                no Tailwind, so utility classes here silently do nothing (the
                unsized svg was the "invisible bell" bug). */}
            <button
                onClick={handleOpen}
                aria-label="Notifications"
                style={{ position: 'relative', background: 'none', border: 'none', cursor: 'pointer', padding: '0.5rem', minWidth: '44px', minHeight: '44px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: isTransparent ? 'white' : 'var(--text-primary)', transition: 'color 0.2s' }}
