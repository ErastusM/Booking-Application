...
describe('booking write-threading — locationId', () => {
    ...
    it('refuses an inactive location, a malformed id, and a well-formed-but-unknown id', async () => {
        const { provider, svc, customer } = await setup();
        ...
        // Valid ObjectId shape, but no such location — the common real rejection.
        expect((await book(customer, svc, { locationId: '0123456789abcdef01234567' })).status).toBe(400);
    });

    it('stamps the location on every occurrence of a recurring booking', async () => {
        const { provider, svc, customer } = await setup();
        const loc = await Location.create({ provider: provider._id, name: 'Main', isPrimary: true, isActive: true });
        const res = await book(customer, svc, {
            locationId: loc._id.toString(),
            isRecurring: true, recurrenceType: 'daily', recurrenceEndDate: futureDate(2),
        });
        expect(res.status).toBe(201);
        const rows = await Appointment.find({ provider: provider._id }).select('+locationId');
        expect(rows.length).toBeGreaterThan(1); // a series, not a single booking
        expect(rows.every((r) => String(r.locationId) === String(loc._id))).toBe(true);
    });
});

describe('booking write-threading — multi-service & group creators (symmetric contract)', () => {
    it('multi-service booking records / rejects a location the same way', async () => {
        const { provider, svc } = await setup();
        const loc = await Location.create({ provider: provider._id, name: 'Main', isPrimary: true, isActive: true });
        const otherBiz = await makeProvider();
        const foreign = await Location.create({ provider: otherBiz._id, name: 'Theirs', isPrimary: true, isActive: true });

        const multi = (locationId) => request(app).post('/api/appointments/multi').set(authHeader(provider)).send({
            appointmentDate: DATE, startTime: '10:00', walkInName: 'Walk-in', services: [{ serviceId: svc._id.toString() }], locationId,
        });
        const ok = await multi(loc._id.toString());
        expect(ok.status).toBe(201);
        expect(String(await locOf(ok.body.data._id))).toBe(String(loc._id));
        expect((await multi(foreign._id.toString())).status).toBe(400); // no longer silently dropped
    });

    it('group booking records / rejects a location the same way', async () => {
        const { provider, svc } = await setup();
        const loc = await Location.create({ provider: provider._id, name: 'Main', isPrimary: true, isActive: true });
        const otherBiz = await makeProvider();
        const foreign = await Location.create({ provider: otherBiz._id, name: 'Theirs', isPrimary: true, isActive: true });

        const group = (locationId) => request(app).post('/api/appointments/group').set(authHeader(provider)).send({
            service: svc._id.toString(), appointmentDate: DATE, startTime: '10:00', endTime: '11:00',
            clients: [{ name: 'A' }, { name: 'B' }], locationId,
        });
        const ok = await group(loc._id.toString());
        expect(ok.status).toBe(201);
        const rows = await Appointment.find({ groupId: { $ne: null } }).select('+locationId');
        expect(rows.length).toBe(2);
        expect(rows.every((r) => String(r.locationId) === String(loc._id))).toBe(true);
        expect((await group(foreign._id.toString())).status).toBe(400);
    });
});
...
describe('public provider locations read', () => {
    ...
    it('returns an empty list (not a 500) for a malformed or unknown provider id', async () => {
        expect((await request(app).get('/api/locations/provider/not-an-id')).status).toBe(200);
        const unknown = await request(app).get('/api/locations/provider/0123456789abcdef01234567');
