...
// owner can reframe or undo any time.
...
const PhotoEditor = ({ url, edit, shape: startShape, isCover, onDone, onCancel, onRemove }) => {
    ...
    const [crop, setCrop] = useState(null); // {x, y, w, h, ar} once the photo has loaded
    // Framed by hand? An untouched photo keeps the automatic top-anchored fill,
    // which re-flows by itself if the post shape changes later.
    const [touched, setTouched] = useState(hasCrop(edit));
    ...
    useEffect(() => {
        ...
        im.onload = () => {
            ...
            if (hasCrop(edit)) {
                ...
                setCrop(Math.abs((ratio * c.h) / c.w - ar) / ar < 0.02 ? contain(c) : fitCrop(c, ratio));
            } else setCrop(defaultCrop(ar, ratio));
        };
    ...
    const pan = (dx, dy) => {
        setCrop((c) => c && contain({ ...c, x: c.x - (dx * c.w) / Math.max(1, frame.w), y: c.y - (dy * c.h) / Math.max(1, frame.h) }));
        setTouched(true);
    };
    ...
    useEffect(() => {
        ...
        const onWheel = (e) => {
            ...
            setCrop((c) => c && zoomTo(c, zoomOf(c, ratio) * Math.exp(-e.deltaY * 0.0015), ratio, fx, fy));
            setTouched(true);
        };
    ...
    const onStageKey = (e) => {
        ...
        if (e.key === '+' || e.key === '=') { e.preventDefault(); setCrop((c) => zoomTo(c, zoomOf(c, ratio) * 1.1, ratio)); setTouched(true); }
        if (e.key === '-') { e.preventDefault(); setCrop((c) => zoomTo(c, zoomOf(c, ratio) / 1.1, ratio)); setTouched(true); }
    };
    ...
    const changeShape = (id) => {
        ...
        const r = ratioOf(id);
        setShape(id);
        setCrop((c) => c && (touched ? fitCrop(c, r) : defaultCrop(c.ar, r)));
    };
    const resetCrop = () => { setCrop((c) => c && defaultCrop(c.ar, ratio)); setTouched(false); };
                        ...
                        <>
                            ...
                            <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.8rem' }}>
                                ...
                                <input type="range" min="1" max={MAX_ZOOM} step="0.01" value={zoom} disabled={!crop} data-testid="photo-zoom"
                                    onChange={(e) => { setCrop((c) => c && zoomTo(c, Number(e.target.value), ratio)); setTouched(true); }}
